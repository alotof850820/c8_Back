...
<script lang="ts" setup>
  ...
  function handleDone(name: string, url: string) {
    ...
    // blob:http://localhost:5173/8142125b-b434-451f-b7ab-b72f035210d1

    const content = editor?.getContent() ?? '';
    const val =
      content?.replace(getUploadingImgName(name), `<img data-key="${name}" src="${url}"/>`) ?? '';
    setValue(editor, val);
  }

  function handleNoApiDone(file: File) {
    emit('getRawFile', file);
  }
